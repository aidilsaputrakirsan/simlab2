...
export class PracticumSchedulingRepository implements IPracticumSchedulingRepository {
    ...
    async storePracticumSchedulingEquipmentMaterial(id: number, data: { practicumSchedulingEquipments: { id: number; quantity: number | null; }[]; proposedEquipments: { name: string; quantity: string; }[]; practicumSchedulingMaterials: { id: number; quantity: number | null; }[]; proposedMaterials: { name: string; quantity: string; }[]; }): Promise<ApiResponse> {
