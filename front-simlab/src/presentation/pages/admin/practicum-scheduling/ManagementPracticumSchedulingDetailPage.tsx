...
import { PracticumSchedulingSessionConductedDTO } from '@/application/practicum-scheduling/dto/PracticumSchedulingDTO';
import { toast } from 'sonner';
import ApproveDialog from '@/presentation/components/custom/ApproveDialog';

const ManagementPracticumSchedulingDetailPage = () => {
    ...
    const [selectedSession, setSelectedSession] = useState<PracticumSchedulingSessionConductedDTO>({ session_id: 0, status: false, information: null })
    const handleOpenConductionConfirm = (id: number, status: boolean) => {
        setSelectedSession({ session_id: id, status: status, information: null })
    ...
    const handleConductedStatus = async (information: string) => {
        if (!practicumSchedulingId) return;

        const payload: PracticumSchedulingSessionConductedDTO = {
            ...selectedSession,
            information: information
        }

        const res = await practicumSchedulingService.setSessionConducted(practicumSchedulingId, payload)
    ...
    if (isLoading || !practicumScheduling) return (
    ...
    return (
        <>
                            ...
                            <div className="flex flex-col gap-5">
                                {practicumScheduling.practicumClasses?.map((cls, idx) => (
                                    <Fragment key={cls.id ?? idx}>
                                        ...
                                        <DataTable columns={PracticumScheduleSessionColumn(handleOpenConductionConfirm, user?.role as userRole, practicumScheduling.status === PracticumSchedulingStatus.Approved)} data={cls.practicumSessions || []} loading={false} />
                                    </Fragment>
                                ))}
                            </div>
                        </CardContent>
            ...
            <ApproveDialog onOpenChange={setIsOpenConfirmation} open={isOpenConfirmation} handleSave={handleConductedStatus} title='Konfirmasi Kehadiran' message='Apakah anda yakin ingin melanjurkan proses konfirmasi?' buttonLabel='Konfirmasi' />
        </>
    )
}

export default ManagementPracticumSchedulingDetailPage