...
import { MajorInputDTO } from "../dto/MajorDTO"

const majorRepository = new MajorRepository()

interface useMajorResult {
    isLoading: boolean
    data: Major[]
    currentPage: number
    perPage: number
    ...
    closeModal: () => void
    handleSave: (formData: MajorInputDTO) => Promise<void>
    handleDelete: (id: number) => Promise<void>
}

const useMajor = (dataFetch: 'all' | 'default' = 'default'): useMajorResult => {
    ...
    const getData = useCallback(async () => {
        setIsLoading(true)
        try {
            const response = await majorRepository.getAll({
                page: currentPage,
                per_page: dataFetch == 'all' ? perPage : 99999,
                search: searchTerm,
                sort_by: sortBy,
    ...
    useEffect(() => {
        getData()
    }, [currentPage, dataFetch, perPage, sortBy, sortDirection])
    ...
    const handleSave = async (formData: MajorInputDTO): Promise<void> => {
        let res
        if (editData) {
            res = await majorRepository.updateData(editData, formData)
        } else {
            res = await majorRepository.createData(formData)
        }
    ...
    const handleDelete = async (id: number): Promise<void> => {
        ...
        if (result.isConfirmed) {
            try {
                const res = await majorRepository.deleteData(id)
