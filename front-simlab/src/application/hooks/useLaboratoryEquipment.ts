...
import { LaboratoryEquipmentInputDTO } from "../laboratory-equipment/dto/LaboratoryEquipmentDTO"

const laboratoryEquipmentRepository = new LaboratoryEquipmentRepository()

interface useLaboratoryEquipmentResult {
    isLoading: boolean
    ...
    handleSave: (formData: LaboratoryEquipmentInputDTO) => Promise<void>
    handleDelete: (id: number) => Promise<void>
    handleFilterRuanganLaboratorium: (e: ChangeEvent<HTMLSelectElement>) => void
}

const useLaboratoryEquipment = (dataFetch: 'all' | 'default' = 'default'): useLaboratoryEquipmentResult => {
    const [data, setData] = useState<LaboratoryEquipment[]>([])
    ...
    const getData = useCallback(async () => {
        setIsLoading(true)
        try {
            const response = await laboratoryEquipmentRepository.getAll({
                page: currentPage,
    ...
    const handleSave = async (formData: LaboratoryEquipmentInputDTO): Promise<void> => {
        let res
        if (editData) {
            res = await laboratoryEquipmentRepository.updateData(editData, formData)
        } else {
            res = await laboratoryEquipmentRepository.createData(formData)
        }
    ...
    const handleDelete = async (id: number): Promise<void> => {
        ...
        if (result.isConfirmed) {
            try {
                const res = await laboratoryEquipmentRepository.deleteData(id)
...
export default useLaboratoryEquipment