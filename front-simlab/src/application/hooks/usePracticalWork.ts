...
import { PracticalWorkDTO } from "../practical-work/dto/PracticalWorkDTO"
import { PracticalWorkRepository } from "../../infrastructure/practical-work/PracticalWorkRepository"

const practicalWorkRepository = new PracticalWorkRepository()

interface usePracticalWorkResult {
    isLoading: boolean
    data: PracticalWork[]
    currentPage: number
    perPage: number
    ...
    closeModal: () => void
    handleSave: (formData: PracticalWorkDTO) => Promise<void>
    handleDelete: (id: number) => Promise<void>
    handleFilterStudyProgram: (e: ChangeEvent<HTMLSelectElement>) => void
}

const usePracticalWork = (): usePracticalWorkResult => {
    const [data, setData] = useState<PracticalWork[]>([])
    ...
    const getData = useCallback(async () => {
        setIsLoading(true)
        try {
            const response = await practicalWorkRepository.getAll({
                page: currentPage,
                per_page: perPage,
                search: searchTerm,
                sort_by: sortBy,
    ...
    }, [currentPage, perPage, searchTerm, sortBy, sortDirection, selectedStudyProgram])
    ...
    const handleSave = async (formData: PracticalWorkDTO): Promise<void> => {
        let res
        if (editData) {
            res = await practicalWorkRepository.updateData(editData, formData)
        } else {
            res = await practicalWorkRepository.createData(formData)
        }
    ...
    const handleDelete = async (id: number): Promise<void> => {
        ...
        if (result.isConfirmed) {
            try {
                const res = await practicalWorkRepository.deleteData(id)
...
export default usePracticalWork